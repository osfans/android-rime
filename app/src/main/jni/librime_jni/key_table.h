...
// Thanks to Markus G. Kuhn <mkuhn@acm.org> for the keyval<->Unicode
...
struct keyval_unicode {
  uint16_t keyval;
  uint16_t ucs;
};

static const keyval_unicode keyval_to_unicode_tab[] = {
    ...
    {0x08c9, 0x2243}, /* similarequal ≃ AkeycodePTOTICALLY EQUAL TO */
    ...
    {0x09df, 0x2422}, /* blank ␢ BLANK keycodeBOL */
...
static int RimeGetKeyUnicode(int keycode) {
  int min = 0;
  int max =
      sizeof(keyval_to_unicode_tab) / sizeof(keyval_to_unicode_tab[0]) - 1;
  int mid;

  /* first check for Latin-1 characters (1:1 mapping) */
  if ((keycode >= 0x0020 && keycode <= 0x007e) ||
      (keycode >= 0x00a0 && keycode <= 0x00ff)) {
    return keycode;
  }

  /* patch encoding botch */
  if (keycode == XK_KP_Space) {
    return XK_space & 0x7f;
  }

  /* special vals */
  if ((keycode >= XK_BackSpace && keycode <= XK_Clear) ||
      (keycode >= XK_KP_Multiply && keycode <= XK_KP_9) ||
      keycode == XK_Return || keycode == XK_Escape || keycode == XK_Delete ||
      keycode == XK_KP_Tab || keycode == XK_KP_Enter ||
      keycode == XK_KP_Equal) {
    return keycode & 0x7f;
  }
  ...
  while (max >= min) {
    mid = (min + max) / 2;
    if (keyval_to_unicode_tab[mid].keyval < keycode) {
      min = mid + 1;
    } else if (keyval_to_unicode_tab[mid].keyval > keycode) {
      max = mid - 1;
    } else {
      /* found it */
      return keyval_to_unicode_tab[mid].ucs;
    }
  }
